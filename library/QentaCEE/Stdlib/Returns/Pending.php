...
abstract class Pending extends \QentaCEE\Stdlib\Returns\ReturnAbstract
{
    ...
    public function __construct(
        $returnData,
        $secret,
        $hashAlgo = \QentaCEE\Stdlib\Fingerprint::HASH_ALGORITHM_HMAC_SHA512
    ) {
        //@see QentaCEE_Stdlib_Return_ReturnAbstract::__construct($returnData)
        parent::__construct($returnData);

        $oFingerprintValidator = new \QentaCEE\Stdlib\Validate\Fingerprint(Array(
            self::$SECRET => $secret,
            self::$FINGERPRINT_ORDER_FIELD => 'responseFingerprintOrder',
        ));

        $oFingerprintValidator->setHashAlgorithm($hashAlgo);
        $oFingerprintValidator->setOrderType(\QentaCEE\Stdlib\Validate\Fingerprint::TYPE_DYNAMIC);
